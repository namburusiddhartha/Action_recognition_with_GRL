...
class RawFrameDecode:
    ...
    def __call__(self, results):
        ...
        #test time
        #possf = results['total_frames']//64
        #results['num_clips'] = possf
        #results['frame_inds'] = np.arange(possf*64)

        label = []
        #print(results['label'])
        for i in range(len(results['frame_inds'])//8):
            label.append(results['label'][results['frame_inds'][0] + i*8])

        if(directory[47:52] == "human"):
            label.append(0)
        else:
            label.append(1)
        ...
        #test time
        #actionlabel = np.asarray(label).flatten()
        #totallabel = np.append(actionlabel, 1)
        #print(totallabel.shape)

        #print(results['total_frames'])
