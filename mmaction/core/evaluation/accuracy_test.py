...
def top_k_accuracy(scores, labels, topk=(1, )):
    ...
    for j in range(len(labels)):
        possf = len(labels[j])//64
        frameinds = np.arange(possf*64)


        for h in range(possf):
            label = []
            for g in range(8):
                label.append(labels[j][frameinds[64*h] + g*8])
            finlabels.append(label)
    print("FIN LABELS")
    print(len(finlabels))
    print(len(scores))
    ...
    for k in topk:
        topk_acc_score = 0
        for i in range(len(finlabels)):
            temp = np.zeros((8,1))
            temp[:,0] = finlabels[i][0:8]
            if(type(scores) != list):
                max_k_preds = np.argsort(scores, axis=1)[:, -k:][:, ::-1]
            else:
                max_k_preds = np.argsort(np.expand_dims(scores[i], 0), axis=1)[:, -k:][:, ::-1]
            max_k_preds = np.squeeze(max_k_preds, axis = 1)
            print(max_k_preds)
            print(temp.T)
            if(k == 1):
                for l in range(8):
                    #print(temp[l,0], max_k_preds[0,l])
                    conf_matrix[int(temp[l,0]),int(max_k_preds[0,l])] = conf_matrix[int(temp[l,0]),int(max_k_preds[0,l])] + 1
            match_array = np.logical_or.reduce(max_k_preds == temp.T, axis=0)
            topk_acc_score = topk_acc_score + (match_array.sum() / match_array.shape[0])

        print(topk_acc_score/len(scores))
        if(k == 1):
