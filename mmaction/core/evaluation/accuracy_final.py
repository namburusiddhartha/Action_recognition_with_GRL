...
def top_k_accuracy(scores, labels, topk=(1, )):
    ...
    # labels = np.reshape(labels.shape[0], 64)
    #print(labels.shape)
    ...
    for k in topk:
        ...
        for i in range(len(scores)):
            ...
            if(type(scores) != list):
                ...
                max_k_preds = np.argsort(np.expand_dims(scores[i].reshape((8,8)), 0), axis=1)[:, -k:][:, ::-1]
            #print(max_k_preds)
            #print(temp.T)
            #print(labels[i,:])
            #print(max_k_preds)
            #print(temp)
            match_array = np.logical_or.reduce(max_k_preds == temp.T, axis=1)
            # print(match_array.shape)
            #print(match_array)
            #print(match_array)
            #print(max_k_preds)
            topk_acc_score = topk_acc_score + (match_array.sum() / match_array.shape[1])
